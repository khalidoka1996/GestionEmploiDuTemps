...
for($jour_sem=1;$jour_sem<7;$jour_sem++){
	...
	for($seance=1;$seance<5;$seance++){
		...
		if(empty($_POST["groupe"])&&empty($_POST["td"])&&empty($_POST["section"])&&empty($_POST["semestre"])){
			echo " <script type='text/javascript'>window.alert('Veuillez donner plus de precision sur la recherche');
			document.location.href='index.php';</script>";
		}
		else 
		if(empty($_POST["groupe"])&&empty($_POST["td"])&&empty($_POST["section"]))
		{
			
			if($_POST["filiere"]==288){
				$sql = "select dayname(S.dateSeance) as jour,S.heureSeance ,S.codeProf,ReG.nom as Groupe,
				N.alias as Niveau,A.alias as Activite,M.nom as Module,ReS.nom as salle,ReP.nom as nomProf,
			ReP.prenom as prenomProf 
				...
				where ReG.codeGroupe=".$_POST["semestre"];
			} else {
		...
		if(empty($_POST["groupe"])&&empty($_POST["td"])){
				$sql = "select dayname(S.dateSeance) as jour,S.heureSeance ,S.codeProf,ReG.nom as Groupe,
				N.alias as Niveau,A.alias as Activite,M.nom as Module,ReS.nom as salle,ReP.nom as nomProf,
			ReP.prenom as prenomProf 
				from enseignements E
		...
		$sql = "select dayname(S.dateSeance) as jour,S.heureSeance ,S.codeProf,ReG.nom as Groupe,
			...
			ReP.prenom as prenomProf 
											...
											and dayname(S.dateSeance)='$tab_jours[$jour_sem]' 
											and S.heureSeance=$heureSeance
											where ReG.codeGroupe in (".$_POST["td"].",".$_POST["section"].")";
		}
		else {
			$sql = "select dayname(S.dateSeance) as jour,S.heureSeance ,S.codeProf,ReG.nom as Groupe,
			N.alias as Niveau,A.alias as Activite,M.nom as Module,ReS.nom as salle,ReP.nom as nomProf,
			...
			and dayname(S.dateSeance)='$tab_jours[$jour_sem]'
			and S.heureSeance=$heureSeance
			where ReG.codeGroupe in (".$_POST["groupe"].",".$_POST["section"].",".$_POST["td"].")";
		}
	...
	if ($row_count>0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			echo "<tr><td class='seance'>".$row["Groupe"]."</td><td class='seance'><span style='font-size:12;'> "
					...
					echo "</span></td><td class='seance'><span style='font-size:12;'>".$row["nomProf"]."-".$row["prenomProf"]."</span></td>";
...
echo " <script type='text/javascript'>window.alert('Veuillez donner plus de precision sur la recherche, retour vers la recherche');
			document.location.href='index.php';</script>";
